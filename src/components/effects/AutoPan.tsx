...
export default function AutoPan(props: AutoPanProps) {
  return <EffectShell title="Auto Pan" typeLabel="Modulation" enabled={props.params.enabled} onToggleEnabled={props.onToggleEnabled} onReset={props.onReset} class="w-52 min-w-52"><div class="device-controls-with-footer grid min-h-0 flex-1 gap-3 px-3 py-3">
    <div class="grid grid-cols-2 place-content-center gap-x-5 gap-y-5">
      ...
      <Knob label="Shape" value={props.params["shape"]} valueLabel={percent(props.params["shape"])} resetValue={defaults["shape"]} min={0} max={1} step={0.01} disabled={!props.params.enabled} automationRange={props.automationRangesByParameterId?.get("autopan.shape")} automated={props.automationRangesByParameterId?.has("autopan.shape")} onAutomationSelect={() => props.onAutomationParameterTouch?.("autopan.shape")} onValueChange={(amount) => { props.onManualAutomationOverride?.("autopan.shape"); props.onChange({ ["shape"]: amount }) }} />
