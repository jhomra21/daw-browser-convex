...
export default function AutoFilter(props: AutoFilterProps) {
  ...
  return <EffectShell title="Auto Filter" typeLabel="2x" enabled={props.params.enabled} onToggleEnabled={props.onToggleEnabled} onReset={props.onReset} class="w-[520px] min-w-[520px]"><div class="grid min-h-0 flex-1 grid-cols-[7rem_1fr] gap-3 px-3 py-3">
    ...
    <div class="grid min-h-0 grid-rows-3 divide-y divide-border">
      ...
      <div class="grid grid-cols-5 place-items-center pt-2">
        {knob('LFO Rate', 'autofilter.lfo.rateHz', props.params.lfo.rateHz, defaults.lfo.rateHz, 0.01, 20, 0.01, `${props.params.lfo.rateHz.toFixed(2)} Hz`, (rateHz) => ({ lfo: { ...props.params.lfo, rateHz } }), true)}
