    ...
    snapshot: (bpm, endTimelineSec) => {
      ...
      return {
        ...
        midi: {
          ...
          cc: expression.filter((entry) => entry.kind === 'cc').length > 0 ? expression.filter((entry): entry is Extract<RecordedEvent, { kind: 'cc' }> => entry.kind === 'cc').map((entry) => ({ id: entry.id, beat: beat(entry.timeSec), controller: entry.controller, value: entry.value, channel: entry.channel })) : undefined,
          pitchBends: expression.filter((entry) => entry.kind === 'pitch-bend').length > 0 ? expression.filter((entry): entry is Extract<RecordedEvent, { kind: 'pitch-bend' }> => entry.kind === 'pitch-bend').map((entry) => ({ id: entry.id, beat: beat(entry.timeSec), value: entry.value, channel: entry.channel })) : undefined,
          channelPressure: expression.filter((entry) => entry.kind === 'channel-pressure').length > 0 ? expression.filter((entry): entry is Extract<RecordedEvent, { kind: 'channel-pressure' }> => entry.kind === 'channel-pressure').map((entry) => ({ id: entry.id, beat: beat(entry.timeSec), value: entry.value, channel: entry.channel })) : undefined,
          polyPressure: expression.filter((entry) => entry.kind === 'poly-pressure').length > 0 ? expression.filter((entry): entry is Extract<RecordedEvent, { kind: 'poly-pressure' }> => entry.kind === 'poly-pressure').map((entry) => ({ id: entry.id, beat: beat(entry.timeSec), pitch: entry.pitch, value: entry.value, channel: entry.channel })) : undefined,
        },
      }
    },
    eventCount: count,
