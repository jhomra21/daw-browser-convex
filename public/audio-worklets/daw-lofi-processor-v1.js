    ...
    for (let channelIndex = 0; channelIndex < output.length; channelIndex++) {
      ...
      for (let frame = 0; frame < target.length; frame++) {
        ...
        if (channel.phase >= channel.interval) {
          ...
          channel.interval = 1 + (this.random() - 0.5) * jitter
          const bits = Math.round(this.parameter(parameters, 'lofi.bitDepth', frame))
          const levels = 2 ** (bits - 1) - 1
          const lsb = 1 / levels
          const noise = (this.random() * 2 - 1) * 10 ** (this.parameter(parameters, 'lofi.noiseDb', frame) / 20)
          let sample = dry + noise
          if (this.state.dither === 'rectangular') sample += (this.random() - 0.5) * lsb
          else if (this.state.dither === 'triangular') sample += (this.random() - this.random()) * lsb
