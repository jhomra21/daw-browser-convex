...
export default function Flanger(props: FlangerProps) {
  const knob = (label: string, id: string, value: number, resetValue: number, min: number, max: number, step: number, valueLabel: string, update: (value: number) => Partial<FlangerParams>, bipolar = false) => (
  ...
  return <EffectShell title="Flanger" typeLabel="Modulation" enabled={props.params.enabled} onToggleEnabled={props.onToggleEnabled} onReset={props.onReset} class="w-[430px] min-w-[430px]"><div class="grid flex-1 grid-cols-6 gap-2 px-3 py-2">
    ...
    {knob('Rate', 'flanger.rateHz', props.params.rateHz, defaults.rateHz, 0.01, 20, 0.01, `${props.params.rateHz.toFixed(2)} Hz`, (rateHz) => ({ rateHz }))}
