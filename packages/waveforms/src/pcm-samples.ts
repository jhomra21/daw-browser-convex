...
import { MAX_PCM_WINDOW_BYTES } from './pcm-envelope'
...
const validatePage = (page: WaveformPcmPage, input: PcmSampleWindowInput) => {
...
export function createPcmSampleWindowCollector(input: PcmSampleWindowInput) {
  ...
  if (!validNonNegativeInteger(input.startFrame)
    ...
    || frameCount * input.channelCount * Float32Array.BYTES_PER_ELEMENT > MAX_PCM_WINDOW_BYTES) {
