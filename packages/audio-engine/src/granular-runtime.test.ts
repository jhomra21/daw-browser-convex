...
describe('granular runtime transfer contract', () => {
  ...
  test('acks transfers, deduplicates identity, freezes, releases, faults, and closes idempotently', async () => {
    ...
    const install = runtime.installSample({ assetKey: 'asset', buffer, sourceStartFrame: 0 })
    ...
    await install
    await runtime.installSample({ assetKey: 'asset', buffer, sourceStartFrame: 0 })
  ...
  test('rejects oversize, install errors, stale acknowledgements, and pending work on close', async () => {
    ...
    await expect(runtime.installSample({ assetKey: 'large', buffer: oversized, sourceStartFrame: 0 })).rejects.toThrow('byte limit')
    const buffer = new AudioBuffer({ length: 4, numberOfChannels: 2, sampleRate: 48_000 })
    const failed = runtime.installSample({ assetKey: 'asset', buffer, sourceStartFrame: 0 })
    ...
    const pending = runtime.installSample({ assetKey: 'asset-2', buffer, sourceStartFrame: 0 })
    runtime.close()
