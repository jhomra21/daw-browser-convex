...
export default function Tremolo(props: TremoloProps) {
  return <EffectShell title="Tremolo" typeLabel="Modulation" enabled={props.params.enabled} onToggleEnabled={props.onToggleEnabled} onReset={props.onReset} class="w-52 min-w-52"><div class="device-controls-with-footer grid min-h-0 flex-1 gap-3 px-3 py-3">
    <div class="grid grid-cols-2 place-content-center gap-x-5 gap-y-5">
      ...
      <Knob label="Shape" value={props.params.shape} valueLabel={percent(props.params.shape)} resetValue={defaults.shape} min={0} max={1} step={0.01} disabled={!props.params.enabled} automationRange={props.automationRangesByParameterId?.get("tremolo.shape")} automated={props.automationRangesByParameterId?.has("tremolo.shape")} onAutomationSelect={() => props.onAutomationParameterTouch?.("tremolo.shape")} onValueChange={(shape) => { props.onManualAutomationOverride?.("tremolo.shape"); props.onChange({ shape }) }} />
