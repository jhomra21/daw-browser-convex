...
At the merge-readiness review, the committed `origin/master...HEAD` branch changes **258 files**, with **29,241 additions** and **3,004 deletions**. The separate current uncommitted review fixes change **21 tracked files**, with **961 additions** and **459 deletions**.
...
- [Start here: what this product is](#start-here-what-this-product-is)
  - [A studio mental model](#a-studio-mental-model)
- [The story of the system on one page](#the-story-of-the-system-on-one-page)
  - [The beginner reading lane](#the-beginner-reading-lane)
  - [The optional engineering deep-dive lane](#the-optional-engineering-deep-dive-lane)
...
- [Advanced sound design](#8-advanced-sound-design)
...
flowchart LR
  ...
  Source -. detector control only .-> Detector["Compressor on bass<br/>effectInstanceId"]
  Bass["Bass track audio<br/>targetTrackId"] --> Detector
  Detector --> Target["Bass output"]
```
...
| Effect draft update | `O(E)` for ordered chain application | `O(E)` chain snapshot | Cloud persistence accepts more than 16 uniquely identified instances |
...
### Live worklet ownership and chain replacement

Track and master chains share one AudioEngine-owned budget of 64 live static AudioWorklets. Native-node effects do not consume this budget, and offline rendering has its separate 256-processor limit. A chain rebuild first reserves only newly constructed worklets. The reservation includes both committed owners and every pending candidate build, so competing rebuilds are rejected synchronously before they exceed the aggregate limit. A successful current revision commits its new count; failed, stale, cleared, or disposed revisions release only their own pending reservation.

The live chain remains committed until all asynchronous candidate resources finish building. Candidate meters, automation bindings, sidechain targets, and routing are isolated until commit. The routing transition then switches to the candidate chain before replaced resources are closed. This prevents a failed second worklet or a stale completion from publishing a partial chain.

### Samplers, sends, sidechains, and history restore

The effects-panel controller owns one sampler buffer cache and synchronization service for both the active instrument and inactive instrument tracks. It installs one runtime listener pair per owned audio engine, keeps lazy sampler configurations available for note-miss loading, pins assets while they are used, and ignores pending loads after disposal.

Sends are ordinary track-to-Return-track routes, with pre-FX, pre-fader, or post-fader taps. Return tracks are named `Return N`; output routing is separate from sends. Invalid or stale sends to non-Return tracks are normalized away and local reload preserves valid sends. Durable sidechain validation is strict. During live hydration an unresolved endpoint is temporarily skipped, then the exact track and effect identities reconnect when available.

Undo restores an effect chain, plus its optional instrument and arpeggiator, in one local IndexedDB transaction or one shared durable operation. The wider track recreation sequence remains ordered rather than becoming one all-encompassing transaction. `restoreUngroup` retains its own explicitly scoped atomic behavior.
