...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[closed=]:duration-250 data-[expanded=]:duration-250 data-[expanded=]:animate-in data-[closed=]:animate-out",
  {
    variants: {
      position: {
...
const SheetContent = <T extends ValidComponent = "div">(
  ...
  return (
    <SheetPortal position={local.position}>
      ...
      >
        {local.children}
        <SheetPrimitive.CloseButton class="absolute right-4 top-4 opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
          <svg
