...
type DecodeAudioFilePagesOptions = {
  startSec?: number
  endSec?: number
  pageFrames?: number
  signal?: AbortSignal
...
export async function* decodeAudioFilePages(
  file: File,
  options: DecodeAudioFilePagesOptions = {},
  ...
  try {
    ...
    if (!(await input.canRead())) throw new Error('Audio file has an unsupported or unrecognizable format.')
    const track = await input.getPrimaryAudioTrack()
    if (!track) throw new Error('File does not contain an audio track.')
