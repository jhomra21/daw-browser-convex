    ...
    <DashboardScrollView>
      <DashboardSection title="VST3 Plug-ins" description="Discovery and preflight are machine-local. Eligible effects can be activated on compatible stereo audio tracks through the native graph; browser playback remains unsupported.">
        <DashboardRow
          ...
          action={<button type="button" class="h-8 border border-border px-3 text-sm disabled:opacity-50" disabled={busy()} onClick={() => void update(() => bridge?.chooseDirectory() ?? Promise.resolve({ ok: false, error: "The desktop plug-in catalog is unavailable." }))}>Add directory</button>}
        />
        ...
        <DashboardRow
          ...
          action={<button type="button" class="h-8 border border-border px-3 text-sm disabled:opacity-50" disabled={busy()} onClick={() => void update(() => bridge?.scan() ?? Promise.resolve({ ok: false, error: "The desktop plug-in catalog is unavailable." }))}>Rescan</button>}
        />
