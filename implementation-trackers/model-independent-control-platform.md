...
| Desktop `registration-v1.json` | machine discovery contract | `apps/desktop/main.ts`, `packages/desktop-protocol/src/client.ts` | Desktop registration schema plus public protocol-client discovery | D | Strict, private, size-bounded, path-contained registration is validated before socket connection. V1/V2 handshake fixtures and packaged adversarial registration cases cover discovery. | Existing contract retained and externally validated | `packages/desktop-protocol/src/client.test.ts`, `packages/control-cli/src/index.test.ts` | Keep registration-v1 until replacement conformance proof |
