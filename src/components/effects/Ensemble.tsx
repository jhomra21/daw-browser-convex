...
export default function Ensemble(props: EnsembleProps) {
  const knob = (label: string, id: string, value: number, resetValue: number, min: number, max: number, step: number, valueLabel: string, update: (value: number) => Partial<EnsembleParams>) => (
  ...
  return <EffectShell title="Ensemble" typeLabel="3 Voices" enabled={props.params.enabled} onToggleEnabled={props.onToggleEnabled} onReset={props.onReset} class="w-[370px] min-w-[370px]"><div class="grid flex-1 grid-cols-5 gap-2 px-3 py-2">
    ...
    {knob('Rate', 'ensemble.rateHz', props.params.rateHz, defaults.rateHz, 0.05, 5, 0.01, `${props.params.rateHz.toFixed(2)} Hz`, (rateHz) => ({ rateHz }))}
