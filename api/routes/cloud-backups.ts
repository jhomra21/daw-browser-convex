...
const createOwnedProjectForBackup = async (
  ...
  await convex.mutation(convexApi.projects.createOwnedRoom, { projectId, userId, serverSecret })
}
...
const deleteCloudProjectAsOwner = async (
  ...
  await convex.mutation(convexApi.projects.deleteRoomAsOwner, {
    projectId,
    userId,
    serverSecret,
  })
...
export function registerCloudBackupRoutes(app: App) {
  ...
  try {
    ...
    try {
      ...
      if (!projectExists) {
        await createOwnedProjectForBackup(convex, projectId, user.id, c.env.CLOUD_PROJECTS_SERVICE_TOKEN)
        createdProject = true
  ...
  try {
    ...
    const convex = new ConvexHttpClient(c.env.VITE_CONVEX_URL)
    const result = await deleteCloudProjectAsOwner(convex, projectId, user.id, c.env.CLOUD_PROJECTS_SERVICE_TOKEN)
